...
class ResidualBlock(nn.Module):
    def __init__(self, device, in_channels, out_channels, stride=1, downsample=None):
...
class ResNet_1(nn.Module):
    def __init__(self, device, block, layers, num_classes=10):
    ...
    def make_layer(self, block, out_channels, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.device, self.in_channels, out_channels, stride, downsample))
        self.in_channels = out_channels
        for i in range(1, blocks):
            layers.append(block(self.device, out_channels, out_channels))
